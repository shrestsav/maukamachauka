              ...
              <tr v-for="(item,index) in brands">
                ...
                <td>
                  <template v-if="editExisting(item.id)">
                    <img :src="brand.logo_src" class="rounded-circle" @click="triggerImageInput(index)" :class="{'img-not-validated':errors.logo_file}"  height="100px">
                    <input type="file" class="custom-file-input" lang="en" v-on:change="imageChange" style="display: none;" :ref="'logo_file_'+index">
                    <div class="invalid-feedback" style="display: block;" v-if="errors.logo_file">
                      {{errors.logo_file[0]}}
                    </div>
                  </template>
                  <img v-else :src="item.logo_src" height="100px">
                </td>
                <td>
                  <div class="form-group" v-if="editExisting(item.id)">
                    <textarea v-model="brand.description" :class="{'not-validated':errors.description}" class="form-control" rows="3" placeholder="BRIEF DESCRIPTION OF Brand"></textarea>
                ...
                <td>
                  <div v-if="editExisting(item.id)">
                    <button type="button" class="btn btn-success btn-sm" @click="saveEdited">Update</button>
                    <button type="button" class="btn btn-info btn-sm" @click="cancelEdit">Cancel</button>
                  </div>
                  <a href="javascript:;" class="table-action" title="Edit Brand" @click="edit(index)" v-if="!modifyBrand.edit">
                    <i class="fas fa-user-edit"></i>
                  </a>
                  <a href="javascript:;" class="table-action" title="Delete New Brand" @click="deleteNew(item.id)" v-if="!modifyBrand.edit && item.can_delete">
  ...
  export default{
    ...
    methods:{
      ...
      saveEdited(){
        ...
        axios.post('/brands/'+this.brand.id,formData)
        .then((response) => {
